...
from examples.dev.autonmt_v2old import AutonmtTranslatorV2
###
import datetime
...
# Constants
DS_LANGS = ["en-es", "en-fr", "en-de", "en-cs"]

def filter_train_fn(src_lines, trg_lines, filter_langs, DS_LANGS, prob_old_tr):
    ...
    new_samples_by_lang = {lang: round(count * coeff) for lang, count, coeff, in
                           zip(datasets.keys(), samples_by_lang, coeff_by_lang)}
    lines = []
...
if __name__ == "__main__":
    for p in [0.0]:  #[0.0, 0.01, 0.05, 0.1, 0.25]
        main(p)
    print("Done!")