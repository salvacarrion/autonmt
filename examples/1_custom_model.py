...
def main():
    ...
    for ds in tr_datasets:
        try:
            ...
            model = Transformer(encoder_embed_dim=512, decoder_embed_dim=512,
                                src_vocab_size=len(src_vocab), trg_vocab_size=len(trg_vocab), padding_idx=src_vocab.pad_id)

            # Train model
            run_prefix = "model512emb"
            model = AutonmtTranslator(run_prefix=run_prefix, model=model, src_vocab=src_vocab, trg_vocab=trg_vocab, force_overwrite=True)
