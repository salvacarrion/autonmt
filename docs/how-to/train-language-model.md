...
trainer = AutonmtCausalLM.from_corpus(corpus, run_prefix="lm", model=model)
...
print(trainer.generate("Once upon a time", strategy=TopPSampling(top_p=0.9), max_new_tokens=64))
```
...
- **Sweep like any grid.** Add `vocab_sizes`/`subword_models` to `encoding`, or declare several
  corpora, and loop over `builder.get_train_ds()` — one `AutonmtCausalLM` per cell — exactly as in
...
- **Pretrained instead of from scratch?** To fine-tune a Hub checkpoint (GPT-2, Llama…)
  rather than train `GPT` from zero, swap `AutonmtCausalLM` for
