...
AutoNMT automates the repetitive half of seq2seq research — tokenization, training, scoring, logging, plotting, file management — so you can focus on the model. Declare a grid of datasets × language pairs × subword models × vocab sizes, and AutoNMT runs the cross-product, persists every intermediate artifact on disk, and produces a single comparable report at the end.

The same script can train AutoNMT's own PyTorch Lightning models, fine-tune HuggingFace seq2seq checkpoints, or shell out to Fairseq — backends are swapped by changing one class.
