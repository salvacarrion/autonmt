...
_BASE_RNNS = {"rnn": nn.RNN, "lstm": nn.LSTM, "gru": nn.GRU}
...
def _last_token(y):
    ...
    ``(batch, t)`` history during teacher-forcing. The RNN cells only need the
    last step.
    """
    if y.dim() == 1:
...
class SimpleRNN(LitSeq2Seq):
    ...
    def get_base_rnn(base_rnn):
        ...
        if key not in _BASE_RNNS:
            raise ValueError(f"Unknown base_rnn '{base_rnn}'. Expected one of: {sorted(_BASE_RNNS)}")
        return _BASE_RNNS[key]
