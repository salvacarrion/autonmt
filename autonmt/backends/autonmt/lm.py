"""PyTorch-Lightning engines for AutoNMT's native language models.

Two sibling engines over a shared base (:class:`_AutonmtLMBase`), one per LM
family:

  * :class:`AutonmtCausalLM` — decoder-only LM: ``fit`` / ``evaluate`` (perplexity)
    / ``generate``.
  * :class:`AutonmtMaskedLM` — encoder-only masked LM: ``fit`` / ``evaluate``
    (masked-token accuracy) / ``fill_mask``.

They are siblings, not parent/child: an MLM is *not* a causal LM (it doesn't
decode left to right), so ``generate`` lives only on the causal engine and
``fill_mask`` only on the masked one — neither overrides the other to raise.

Deliberately *not* :class:`~autonmt.backends._base.translation_engine.BaseTranslator`
subclasses: that class's ``predict()`` is translate-and-score (eval datasets,
beams, BLEU, src/ref/hyp), none of which maps onto an LM. The base composes
:class:`RunLayout` for paths and :func:`manual_seed` for reproducibility, and
reuses :class:`~autonmt.backends._base.config.FitConfig` for the training
...
from autonmt.core.decoding import LMGenerator
...
class _AutonmtLMBase(ABC):
    """Shared fit / run-layout / checkpoint scaffolding for the native LM engines.

    Subclasses implement :meth:`_make_datasets` (which family of torch dataset to
    pack) and an :meth:`evaluate` suited to the objective; the decoder-only and
    encoder-only verbs (``generate`` / ``fill_mask``) live on the subclasses.
    """
    ...
    def from_corpus(cls, corpus, *, run_prefix, model, **kwargs):
        """Bind an engine to ``corpus``'s runs path (mirrors ``BaseTranslator.from_dataset``)."""
        return cls(
            model=model,
    ...
    def _make_datasets(self, corpus, block_size):
        """Build the ``(train, val)`` torch datasets :meth:`fit` trains on."""
        ...
...
class AutonmtCausalLM(_AutonmtLMBase):
    """Train / evaluate / sample from a decoder-only LM on an :class:`LMCorpus`."""

    def _make_datasets(self, corpus, block_size):
        """Build the ``(train, val)`` causal datasets (packed blocks; supervise
        mask for instruct corpora, ``None`` for pure-LM)."""
        return (
    ...
    def generate(self, prompt, strategy=None, max_new_tokens=64, corpus=None,
        ...
        out_ids = LMGenerator(strategy=strategy).generate(
...
class AutonmtMaskedLM(_AutonmtLMBase):
    ...
    def _make_datasets(self, corpus, block_size):
        if corpus.mask_id is None:
            raise ValueError(
                "AutonmtMaskedLM needs a corpus built with mode='mlm' (which reserves a "
